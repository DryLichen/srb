...
export const constantRoutes = [
  ...
  //插入积分等级管理的template
  {
    path: '/core/integral-grade', // 路由地址，访问该地址时定位到组件锚点
    component: Layout, //布局组件
    redirect: '/core/integral-grade/list', // 重定向地址
    name: 'coreIntegralGrade', //唯一的路由标识名
    meta: { title: '积分等级管理', icon: 'el-icon-s-marketing' }, //控制页面显示的标题与图标
    alwaysShow: true, //只有一个子节点时如果false就会不显示父节点
    children: [
      {
        path: 'list',
        name: 'coreIntegralGradeList',
        component: () => import('@/views/core/integral-grade/list'), //引入template模板组件
        meta: { title: '积分等级列表' },
      },
      {
        path: 'create',
      ...
      {
        path: 'edit/:id', //占位符，表示任意id
        name: 'core',
        component: () => import('@/views/core/integral-grade/form'),
        meta: { title: '编辑积分等级' },
        hidden: true, //因为编辑积分等级不需要单独列出，而是由列表页的按钮控制，所以可以隐藏
      },
    ],
  },
  ...
  //会员管理
  {
    ...
    children: [
      {
        path: 'list',
        component: () => import('@/views/core/user-info/list.vue'),
  ...
  // 借款管理
  {
    ...
    children: [
      ...
      },
      {
        ...
        path: 'detail/:id', //:id 是一个占位符，表示这部分url会是任何一个id'
