<template>
  ...
  <div class="item-detail wrap">
    ...
    <div class="item-detail-head clearfix" data-target="sideMenu">
      ...
      <!-- 投资表单 -->
      <!-- 禁止借款人看到此表单 and 只有募资中标的可以展示投资信息 -->
      <div v-if="userType === 1 && lend.status === 1" class="bd clearfix">
        <div class="data" style="width: auto;">
          <el-form :inline="true" class="demo-form-inline">
            <el-form-item label="投资金额">
              <el-input v-model="invest.investAmount" :disabled="lend.status != 1" @blur="getYield()">
    ...
    <!-- 回款计划 -->
    <div v-if="userType === 1 && isInvestor" class="item-detail-body clearfix mrt30 ui-tab">
      <div class="ui-tab-nav hd">
        <ul>
          <li class="nav_li active">
...
export default {
  ...
  async asyncData({ $axios, params }) {
    let lendId = params.id  //通过路由参数获取id

    let response = await $axios.$get(`/api/core/lend/show/${lendId}`) //同步获取
  ...
  methods: {
    ...
    fetchAccount() {
      ...
      if(userInfo){
        this.$axios.$get('/api/core/userAccount/auth/amount')
          .then( response => {
    ...
    //计算收益
    getYield() {
      this.$axios.$get(`/api/core/lend/getYield/${this.invest.investAmount}/${this.lend.lendYearRate}/${this.lend.period}/${this.lend.returnMethod}`)
        .then( response => {
          this.interestCount = response.data.yield
        })
    },
    ...
    commitInvest() {
      ...
      //判断标的是否超卖
      let sum = Number(this.invest.investAmount) + this.lend.investAmount
      if(sum > this.lend.amount) {
        this.$message.error('可投金额不足')
        return
      }
      ...
      //判断投资金额是否为100整数倍
      if(this.invest.investAmount == 0 || this.invest.investAmount % this.lend.lowestAmount != 0){
        this.$message.error(`投资金额必须为${this.lend.lowestAmount}整数倍`)
        return
      }

      //向后端发起投标请求
      this.$alert('<div style="size: 18px;color: red;">您即将前往汇付宝确认标的</div>',
          '前往汇付宝资金托管平台',
          {
            ...
            callback: (action) => {
              if(action == 'confirm'){
                this.invest.lendId = this.lend.id //给invest赋值
    ...
    //获取回款计划数据
    fetchLendItemReturn(){
      this.$axios.$get(`/api/core/lendItemReturn/auth/list/${this.$route.params.id}`)
        .then( response => {
          this.lendItemReturnList = response.data.result.list
          this.isInvestor = response.data.result.isInvestor
        })
    },
    ...
    commitReturn(lendReturnId){
      this.$alert(
          ...
          '前往汇付宝资金托管平台',
          {
              ...
              callback: action =>{
                if(action == 'confirm'){
                  this.$axios.$get(`/api/core/lendReturn/auth/commitReturn/${lendReturnId}`)
                    .then( response => {
                      document.write(response.data.form)
                    })
                }
